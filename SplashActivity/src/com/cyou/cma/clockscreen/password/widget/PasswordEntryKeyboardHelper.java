...
public class PasswordEntryKeyboardHelper implements OnKeyboardActionListener {
    ...
    public PasswordEntryKeyboardHelper(Activity activity, KeyboardView keyboardView, EditText targetView) {
        mActivity = activity;
...
// mVibrator = new Vibrator();
    ...
    private void createKeyboards() {
        mNumericKeyboard = new PasswordEntryKeyboard(mActivity, R.xml.password_kbd_numeric);
        mQwertyKeyboard = new PasswordEntryKeyboard(mActivity,
                R.xml.password_kbd_qwerty, R.id.mode_normal);
        mQwertyKeyboard.enableShiftLock();

        mQwertyKeyboardShifted = new PasswordEntryKeyboard(mActivity,
        ...
        mSymbolsKeyboard = new PasswordEntryKeyboard(mActivity, R.xml.password_kbd_symbols);
        mSymbolsKeyboard.enableShiftLock();

        mSymbolsKeyboardShifted = new PasswordEntryKeyboard(mActivity,
    ...
    private void sendKeyEventsToTarget(int character) {
        Editable editable = mTargetView.getText();
        int start = mTargetView.getSelectionStart();
        editable.insert(start, Character.toString((char) character));
...
// }
    ...
    public void setVibratePattern(int id) {
        int[] tmpArray = null;
        try {
            tmpArray = mActivity.getResources().getIntArray(id);
