...
public class SwipeDeleteListView extends ListView {
	...
	private void handleActionUp(MotionEvent ev) {
		...
		if (dismiss) {
			...
			// }
			// });
			final boolean toRight = dismissRight;
			ViewPropertyAnimator.animate(mDownView)
					...
					.setListener(new AnimatorListenerAdapter() {
						...
						public void onAnimationEnd(Animator animation) {
							...
							performDismiss(mDownView, mDownPosition, toRight);
						}
					});
		} else {
			// 将item滑动至开始位置
	...
	public void resetAllItem() {
		...
		for (int i = 0; i < itemCount; i++) {
			v = getChildAt(i);
			if (v != null) {
				ViewPropertyAnimator.animate(v).translationX(0).alpha(1)
						.setDuration(mAnimationTime).setListener(null);
			}
		}
	}
		...
		for (int i = 0; i < itemCount; i++) {
			...
			if (v != null) {
				ViewPropertyAnimator
						...
						.setListener(
								i == itemCount - 1 ? mRemoveAllListener : null)
						.setStartDelay(delayTime);
				delayTime += mAnimationTime;
			}
		}

	}

	private AnimatorListenerAdapter mRemoveAllListener = new AnimatorListenerAdapter() {
		@Override
		public void onAnimationEnd(Animator animation) {
			onDismissCallback.onRemoveAllItem();
		}
	};
	...
	private void performDismiss(final View dismissView,
			final int dismissPosition, final boolean toRight) {
		...
		animator.addListener(new AnimatorListenerAdapter() {
			...
			public void onAnimationEnd(Animator animation) {
				if (onDismissCallback != null) {
					if (toRight) {
						onDismissCallback.onOpen(dismissPosition);
					} else {
