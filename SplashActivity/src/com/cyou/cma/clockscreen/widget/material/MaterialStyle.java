...
public class MaterialStyle {
	...
	public void doDraw(Canvas canvas) {
		...
		if (mBackgroundDrawable != null) {
			...
			// mBackgroundDrawable.setState(mBackgroundDrawable.getState());
			int state[] = mBackgroundDrawable.getState();
			if (mBackgroundDrawable instanceof StateListDrawable) {
				StateListDrawable stateListDrawable = (StateListDrawable)mBackgroundDrawable;
				stateListDrawable.setState(mDependView.getDrawableState());
		...
		case STATUS_ANIM_FAST:
			...
			case TYPE_HEADBAR_BACK_BTN:
				if (mBackgroundDrawable == null && mBackgroundColor == 0) {
					...
					if (VERSION.SDK_INT >= VERSION_CODES.ICE_CREAM_SANDWICH) {
						...
						mPath.addRect(0, 0, mCircleRadius, getHeight(),
								Direction.CCW);
					}
			...
			case TYPE_HEADBAR_RIGHT_BTN:
				if (mBackgroundDrawable == null && mBackgroundColor == 0) {
					mPath.reset();
					if (VERSION.SDK_INT >= VERSION_CODES.ICE_CREAM_SANDWICH) {
						mPath.addCircle(getWidth(), getHeight() / 2,
								mCircleRadius, Direction.CCW);
					} else {
						mPath.addRect(getWidth() - mCircleRadius, 0,
								getWidth(), getHeight(), Direction.CCW);
					}
			...
			case TYPE_NORMAL_BTN:
				if (mBackgroundDrawable == null && mBackgroundColor == 0) {
					mPath.reset();
					if (VERSION.SDK_INT >= VERSION_CODES.ICE_CREAM_SANDWICH) {
						mPath.addCircle(actionDownX, actionDownY,
								mCircleRadius, Direction.CCW);
					} else {
						mPath.addRect(actionDownX - mCircleRadius, 0,
								actionDownX + mCircleRadius, getHeight(),
								Direction.CCW);
					}
